...
def wavepacket_ic(solver, domain, params):
    ...
    uz.differentiate('z', out=ux)
    ux['g'] = np.roll(-(ux['g'] + uz['g'] / (2 * params['H'])) / params['KX'],
                      shift, axis=0)
...
def setup_problem_unforced(problem, domain, params):
    ...
    problem.add_equation('dx(ux) + dz(uz) + uz / (2 * H) = 0')
    problem.add_equation(
        'dt(rho)'
        '+ sponge * rho'
        '- rho0 * uz / H' +
        # '= -ux * dx(rho) - uz * dz(rho) + uz * rho / (2 * H)'
        '= 0'
    )
    problem.add_equation(
        'dt(ux)'
...
def load(get_solver, setup_problem, name, params):
    ...
    state_vars['E'] = (state_vars['ux']**2 + state_vars['uz']**2) / 2
