...
\begin{equation}
    ...
    \begin{cases}
        ...
        e^{(z - z_0)/2H}e^{i(k_xx - k_z(z - z_0) - \omega t)
            + \frac{1}{2k_zH}} & z < z_0\\
    \end{cases}.\label{eq:anal_forced}
\end{equation}
...
\subsection{Narrow-Band Forcing: Exact}

We use variation of parameters. We start with PDE in $\varpi, \Upsilon$
coordinates and transform to ODE
\begin{align}
    \rd{u_{z}'}{z} + ik_xu_x' &= 0,\nonumber\\
...
\end{align}
We have divided through by $e^{i(k_xx - \omega t)}$ and taken $u_z'(x, z, t)
\Rightarrow u_z'(z)$. The inhomogeneous solutions are most easily written
\begin{equation}
    u_{z,\pm}'(z) = e^{\p*{\frac{1}{2H} \pm ik_z}\p*{z - z_0}},
\end{equation}
in which case the Wronskian is
\begin{equation}
...
The general solution to \autoref{eq:narrow_inhomo} is then given $u(z) = A_+u_+
+ A_-u_-$ where
\begin{align}
    A_{\pm} &= \mp \int \frac{1}{W} u_{\mp} \p*{-\frac{gk_x^2}{\omega^2}
            Ce^{-\frac{(z - z_0)^2}{2\sigma^2}}}\;\mathrm{d}z,\nonumber\\
        &= \mp \frac{1}{2ik_z}\frac{gk_x^2}{\omega^2}\int \exp\s*{
            \p*{-\frac{1}{2H} \mp ik_z}z' - \frac{z'^2}{2\sigma^2}}
            \;\mathrm{d}z',\nonumber\\
        &= \mp \frac{1}{2ik_z}\frac{gk_x^2}{\omega^2}\int \exp\s*{
            -\frac{z'^2 + \p*{\frac{\sigma^2}{H} \pm 2ik_z\sigma^2}z'
            }{2\sigma^2}}\;\mathrm{d}z',\nonumber\\
        &= \mp \frac{1}{2ik_z}\frac{gk_x^2}{\omega^2}\int \exp\s*{
            -\frac{\p*{z' + \frac{\sigma^2}{2H} \pm ik_z\sigma^2}^2
                - \p*{\frac{\sigma^2}{2H} \pm ik_z\sigma^2}^2
            }{2\sigma^2}}\;\mathrm{d}z',\nonumber\\
        &= \mp \frac{1}{2ik_z}\frac{gk_x^2}{\omega^2}
            e^{\frac{\p*{\frac{\sigma^2}{2H} \pm ik_z\sigma^2}^2}{2\sigma^2}}
                \sqrt{2\sigma^2} \int e^{-\xi^2}\;\mathrm{d}\xi,\nonumber\\
        &= \mp \frac{1}{2ik_z}\frac{gk_x^2}{\omega^2}
            e^{\frac{\p*{\frac{\sigma^2}{2H} \pm ik_z\sigma^2}^2}{2\sigma^2}}
                \sqrt{2\sigma^2} \frac{\sqrt{\pi}}{2}\p*{1 + C +
                    \erf\p*{\frac{z - z_0}{\sqrt{2\sigma^2}}
                        + \frac{\sigma}{2H\sqrt{2}}
                        \pm \frac{ik_z\sigma}{\sqrt{2}}}}
\end{align}
We have made $z' \equiv z - z_0, \xi = \s*{ \frac{z - z_0}{\sqrt{2\sigma^2}} +
\frac{\sigma}{2H\sqrt{2}} \pm \frac{ik_z\sigma}{\sqrt{2}}}$, and $\erf(z) =
\frac{2}{\sqrt{\pi}}\int\limits_0^z e^{-t^2}\;\mathrm{d}t$. $C$ is a constant of
integration. Since we want $u(z\to \infty) = u_+, u(z \to -\infty) = u_-$, we
can quickly see that $C_+ = 0, C_- = -2$. Thus, the specific solution can be
written down
\begin{equation}
    u(z) = \frac{1}{2ik_z}\frac{gk_x^2}{\omega^2}
            e^{\frac{\p*{\frac{\sigma^2}{2H} \pm ik_z\sigma^2}^2}{2\sigma^2}}
                \sqrt{2\sigma^2} \frac{\sqrt{\pi}}{2}\s*{
                    -u_+\p*{1 + \erf\p*{\xi}}
                    + u_- \p*{\erf\p*{\xi} - 1}}
\end{equation}
In the limit where $H \to \infty$ and $\erf(\xi) \approx \Theta(\xi)$, we
recover
\begin{equation}
    u(z) = -\frac{1}{2ik_z}\frac{gk_x^2}{\omega^2}e^{-\frac{k_z^2\sigma^2}{2}}
...
\rho_0 \propto e^{-z/H}$. Try ansatz $z(t) = -H\p*{\ln t / \tau}$ and we
find
\begin{align}
    \rho_0(z = 0) \frac{t}{\tau} c_{ph, x} \p*{-\frac{H}{t}} &= -S_{px}
        \nonumber\\
    \tau &= \frac{H\rho_0(z = 0)c_{ph, x}}{S_{px}},\nonumber\\
    z(t) &= -H\ln \frac{t + t_0}{\tau}.
\end{align}
$t_0$ is some constant of integration that is fit by setting $z_c(t = 0) = z_0$
some initial condition. Let's first notice that $z(-t_0) = +\infty, z(\tau -
t_0) = 0$, and so $\tau$ corresponds to the time it takes to spin up the entire
atmosphere above $z = 0$. Indeed, halving $S_{px}$ doubles the amount of time
necessary.

In our simulations, it seems like $S_{px}$ decreases over time due to reflective
behavior setting in (similar to Winters \& d'Asaro, though since they had a very
narrow critical layer they observed transmission too, impossible in our case).
We can thus consider how two $z(t, S_{px})$ solutions differ based on choice of
$S_{px}$ given some fixed initial condition. In our case, since the beginning of
the simulation is dominated by transients, horizontal momentum dissipation and
formation of the critical layer, our equation is only really valid towards the
end of the simulation. Thus, we formulate our problem as follows: given two
$S_{px}, \alpha S_{px}$, compute $z(t)$ for both given initial condition
$z(t_f) = z_f$. We can solve and obtain
\begin{equation}
    t_0 = \tau e^{-z_f/H} - t_f,
\end{equation}
and so the exact solution becomes
\begin{equation}
    z(t) = -H\ln \frac{t - t_f + \tau e^{-z_f/H}}{\tau}.
\end{equation}

\section{Masking Advective Terms}

Recall \autoref{eq:anal_forced} tells us the amplitude of the excited wave in
terms of the forcing function. However, numerically, it appears that the forcing
zone evolves a local mean flow $\bar{U}_0 < 0$ due to the nonlinear terms. The
effect of such a term is to replace $\omega_0 \to \omega_0 - k_x\bar{U}_0 >
\omega_0$ in \autoref{eq:anal_forced}, which has the effect of attenuating the
forcing amplitude by $\sim \frac{1}{1 + \frac{\bar{U}_0}{c_{ph, x}}} \approx
\p*{1 - \frac{\bar{U}_0}{c_{ph, x}}}$.

The cause of this term comes from the viscous terms, which $\sim \partial_t -
\nu \nabla^2$ (our particular implementation differs slightly). In the
$\delta$-function forcing limit, $u_x, u_z$ are only $\mathcal{C}^0$ at $z_0$.
The derivative jumps at $z_0$ with height dictated by the forcing strength.
Thus, $\nabla^2 u_x, \nabla^2 u_z \sim \delta(z - z_0)$, and there is a strong
extra contribution to the EOM at $z_0$.

\section{Estimating Various Other Scales}

\subsection{Viscous Damping Length}

This is a rather hacky derivation. Consider that in the fluid equations, if we
assume $\frac{1}{k_zH} \ll 1$, then viscosity basically just enters as a bunch
of $\nu k^2$ terms. We can absorb this into the frequency $\partial_t + \nu k^2
\sim i\omega_0\p*{1 - \frac{i\nu k^2}{\omega_0}}$. Plugging into the dispersion
relation then shows that ($k_0 = \abs*{\vec{k}_0} = \sqrt{k_{x0}^2 + k_{z0}^2}$)
\begin{align*}
    \omega^2 &\approx \frac{N^2k_{x0}^2}{k^2}\p*{1 - \frac{2i\nu k^2}{\omega_0}},\\
        &\approx \frac{N^2k_{x0}^2}{k_{x0}^2 k_{z0}^2\p*{1 + \frac{2i\nu k^4}{\omega
            k_{z0}^2}}},\\
        &\approx \frac{N^2k_{x0}^2}{k_{x0}^2 + \s*{k_{0z}
            \p*{1 + \frac{i\nu k_0^4}{\omega k_{0z}^2}}}},\\
        &\approx \frac{N^2k_{x0}^2}{k_{x0}^2 + \s*{k_{0z}
            \p*{1 + \frac{i\nu k_0^5}{Nk_{x0} k_{0z}^2}}}},\\
    e^{ik_zz} &\approx \exp\s*{ik_{z0}z - \frac{\nu k_0^5}{Nk_{x0}k_{z0}}z}.
\end{align*}
Thus, in the small dissipation limit we can treat the artificial viscosity the
same as a vertical damping length.

\subsection{Energy Dissipation Thickness}

The wave action $I = \frac{E}{\tilde{\omega}} = \frac{E}{\omega_0 -
k_x\bar{U}_0}$ is a conserved quantity for an adiabatically varying shear flow
$\bar{U}_0(z)$. Then, writing $E = I\tilde{\omega}$, we can compute the energy
dissipation as a function of $z$:
\begin{align*}
    \pd{E}{z} &= -Ik_x\pd{\bar{U}_0}{z},\\
        &= -\frac{E_0}{\omega_0} k_x \pd{\bar{U}_0}{z},\\
        &= -\frac{E_0}{N} \abs*{\vec{k}_0} \pd{\bar{U}_0}{z},\\
        &= -E_0 \frac{\abs*{\vec{k}_0}}{\sqrt{\mathrm{Ri}}}.
\end{align*}

In most astrophysical applications, $k_z \gg k_x$, and so we find that the
characteristic dissipation lengthscale is $\sim
\frac{\sqrt{\mathrm{Ri}}}{k_{z0}}$.
