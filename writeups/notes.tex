...
\section{Linear Amplification}

One can solve \autoref{eq:shear_lin_uz}

\section{Breaking Criterion}

If the fluid is not strongly stratified $\ptd{u_0}{z}, \pd{u_0}{z}\frac{1}{H}
\ll \frac{k_xN^2}{\omega_{cr}}$ then the singularity is largely induced by
$\omega_{cr} \to 0$ (note that the condition becomes easier to satisfy as we
approach the singularity). $\omega_{cr} = 0$, is exactly the \emph{corotation
resonance}, and one expects that as (note that if $k_z \gg k_x$ then the
dispersion relation is $\omega_{cr} \approx Nk_x/k_z$):
\begin{equation}
    k_z(z) \approx \frac{Nk_x}{\omega_{cr}(z)}
        \sim \frac{\omega_{cr}(z)}{\abs*{u_{1z}}},
...
z_{\max}$. Consider $\pd{u_0}{z} \sim \frac{\omega}{k_xz_{\max}} \approx
\frac{N}{k_zz_{\max}}$. This both satisfies $\pd{u_0}{z} \frac{1}{H} =
\frac{N}{k_zz_{\max}H} \ll \frac{k_xN^2}{\omega_{cr}}$ (as $k_z^2z_{\max}H \ll
1$), and $\ptd{u_0}{z} = 0 \ll \frac{k_xN^2}{\omega_{cr}}$. We can even induce
the corotation resonance well below $z_{\max}$ and still be perfectly within the
regime of validity of our assumption.
...
\pm \sqrt{k_xNu_{1z}}$ from the nonlinearity criterion
\autoref{eq:corot_nonlin_cond}. Thus, the width of $u_0$ that incurs nonlinear
effects is roughly of order $2\sqrt{Nu_{1z} / k_x}$, which has the right scaling
behavior (a stronger perturbation should be resonant farther away).
