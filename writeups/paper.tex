...
assume no background flow, and $\bm{u} = \bm{u}'$. Physically, this assumption
...
\begin{itemize}
    ...
    \item The time-averaged total $x$-momentum flux $F$ carried in the $\bm{z}$
        ...
        separable as $f(x, z, t) = f(z)e^{i(k_{1x}x - \omega_1 t)}$, $x$
...
on the density equation. We implement forcing with strength $C$ by replacing
\autoref{eq:nl_up_density} with
\begin{equation}
...
rest. Solving for $\overline{U} = \overline{U}_c \equiv \omega / k_x$ the
critical layer condition (see \autoref{ss:crit_layer} immediately below), we
obtain $\abs{\frac{u_{x}'k_{x}}{\omega}} = \abs{\xi_x k_{x}} = \abs{\xi_z k_z}
\sim 1$, equal to the nonlinearity criterion \autoref{eq:nl}. This condition
predicts where the wave's self-acceleration alone produces a critical layer.
...
= 0$, the horizontal wavenumber of the linear wave in the fluid's frame of
reference vanishes and critical behavior is observed. In a linear
...
\begin{equation}
    \pd{}{t}\int\limits \rho(z) \overline{U}(z, t)\;\mathrm{d}z
        + \Delta S = 0.
\end{equation}
$\Delta S$ is the total injected horizontal momentum to the flow. We next make a
...
\begin{equation}
    \rho(z_c) \overline{U}_c\pd{z_c}{t} = \Delta S.\label{eq:zc_anal}
\end{equation}
For constant $\Delta S$ in time and $\rho \approx \overline{\rho}$, this has
analytical solution
\begin{equation}
    z_c(t) = -H\ln t - H\ln \frac{H\overline{\rho}_0\overline{U}_c}{-\Delta S}
        .\label{eq:zc_sol}
\end{equation}
...
We nondimensionalize by taking $H = N = \overline{\rho}_0 = 1$ in
\autoref{se:nl_orig}.
...
\begin{figure*}
    ...
    distinct phases of the evolution of the flow. Note that $\bar{U}_c \approx
    0.16$ for the parameters used.}\label{fig:snapshots}
\end{figure*}
...
introduce $\varpi = \frac{P}{\rho}$ instead, then mandate $\overline{\rho}, \varpi_0$
background fields satisfy hydrostatic equilibrium $\bm{\nabla}\varpi_0 +
\varpi_0 \bm{\nabla}\overline{\rho} + g\bm{z} = 0$. Taking isothermal stratification,
we find $\varpi_0 = gH$. We further change variables to $\Upsilon = \ln \rho -
\ln \overline{\rho}$ and $\varpi_1 = \varpi - \varpi_0$ deviations from the background
...
$\rho = \overline{\rho}, \bm{u} = 0$. Including the damping layers and forcing terms as
