...
$\overline{u}_x(z)\bm{x}$ is permitted in hydrostatic equilibrium, but we will
assume no background flow. Physically, this assumption corresponds to a
non-rotating star, or going to the corotating frame of a rigidly rotating star.

In practice, it is convenient to introduce coordinate $\Upsilon = \ln
\frac{\rho}{\overline{\rho}}$. This both identically enforces $\rho > 0$ and
avoids numerical issues in the $\frac{\bm{\nabla}P}{\rho}$ term if $\rho$ is
small. We also define reduced pressure $\varpi = \frac{P}{\rho}$. Then, we may
rewrite the second two equations of \autoref{se:nl_orig} as
...
$\overline{\Upsilon} = 1, \overline{\varpi} = gH$.
...
\bm{\nabla}$, we may linearize \autoref{se:nl_upsilon}.
%,We ignore the advective components of the material derivative and consider small
% deviations $\Upsilon' = \Upsilon - \overline{\Upsilon}, \varpi' = \varpi -
% \overline{\varpi}$ about hydrostatic equilibrium:
% \begin{subequations}\label{se:lin_homo}
%     \begin{align}
%         \bm{\nabla} \cdot \bm{u}_1 &= 0,\\
%         \pd{\Upsilon_1}{t} - \frac{u_{z}}{H} &= 0,\\
%         \pd{\bm{u}_1}{t} + \bm{\nabla}P_1
%             + \p{1 + \Upsilon_1} g\bm{z}
%             &= 0.
%     \end{align}
% \end{subequations}
The solution in this linear regime up to undetermined amplitude $A$ is
given by \citep{drazin,sutherland0}:
\begin{equation}
...
\end{equation}
the \emph{Brunt-V\"ais\"al\"a frequency} to be constant and take the
incompressible limit $c_s \to \infty$ where $c_s$ is the sound speed. Other
dynamical quantities are simply related to $u'_z$ (see
e.g.\ \cite{sutherland0}).

In the weak stratification limit $k_{z}H \gg 1$, the solution exhibits the
...
\begin{itemize}
    ...
    \item The phase and group velocities can be computed respectively:
        ...
        (e.g.~\cite{drazin,sutherland1}). Note again that $c_{gz} < 0$ for
        $k_{x}, k_{z} > 0$, so upwards-propagating IGW have $k_z < 0$.
...
with
\begin{equation}
    \md{\Upsilon}{t} + u_{z}'\pd{\ln \bar{\rho}}{z}
...
$k_{x}, \omega_(k_{x}, k_{z})$ will propagate.
...
\begin{equation}
    \abs{\bm{\xi}'} \abs{\bm{k}} \gtrsim 1.\label{eq:nl}
\end{equation}
...
(see \autoref{ss:crit_layer} immediately below), we obtain
...
\overline{U}(z)\partial_x$. For a critical value $\omega - \overline{U}_c k_{x}
= 0$, the frequency of the linear wave in the fluid's frame of reference
vanishes and critical behavior is observed. In a linear
\citep{booker_bretherton} or viscous \citep{hazel} theory, the incident wave has
...
the shear flow. Since the shear flow cannot exceed $\overline{U}_c$ the horizontal
...
\begin{equation}
    \pd{}{t}\int\limits_0^{L_z} \rho(z) \overline{U}(z, t)\;\mathrm{d}z
        + \Delta S = 0.
\end{equation}
We define $\Delta S$ to be the change in flux across the critical layer, or
equivalently the negative of the absorbed flux. Treating $\overline{U}(z > z_c) =
\overline{U}_c, \overline{U}(z < z_c) = 0$ gives us exactly
\begin{equation}
    \rho(z_c) \overline{U}_c\pd{z_c}{t} = \Delta S(z_c(t)).\label{eq:zc_anal}
\end{equation}
$\Delta S(z_c)$ denotes the change in $S$ across $z_c$. For constant $\Delta S$
in time and $\rho \approx \bar{\rho}$, this has analytical solution
\begin{equation}
    z_c(t) = -H\ln t - H\ln \frac{H\bar{\rho}_0c_{ph, x}}{-\Delta S}
        .\label{eq:zc_sol}
\end{equation}
...
We nondimensionalize by taking $H = N = \bar{\rho}_0 = 1$ in
\autoref{se:nl_orig}.

% TODO physical vs simulation domain
...
\begin{itemize}
    ...
    \item $\omega$: We choose $\omega$ by evaluating dispersion relation
        ...
        R$, we aim to study $k_{z}H \gg 1$. However, to satisfy this well is
        computationally expensive: $L_z \gg H$ is required to give waves ample
        room to grow within the simulation domain, but we want $k_{z}$ to be
    ...
    \item $\nu$: Nonlinear effects transfer wave energy from $\bm{k}$ to
        ...
        conserve horizontal momentum (see \autoref{ss:strat_impl} for details).
        Under weak forcing, we may set $\nu = 0$, since energy transfer out of
        the parent $\bm{k}$ mode is negligible.
\end{itemize}

Finally, we use initial conditions $\bm{u}(0) = \Upsilon(0) = 0, \varpi(0) =
...
excited. The incompressibility constraint allows us to compute a $\bm{u}(x, z,
t)$ profile analytically. The amplitude of the observed IGW in $\bm{u}$ relative
to analytical solution $\bm{u}_{al}$  can be estimated with estimator
$\hat{A}_i(t)$ (subscript $i$ denotes incident wave)
\begin{equation}
    \hat{A}_i(t) = \frac{\int\limits_{z_b}^{z_t}\int\limits_0^{L_x}
        \bar{\rho}\p{\bm{u} \cdot \bm{u}_{al}}\;\mathrm{d}x\mathrm{d}z}
        {\int\limits_{z_b}^{z_t}\int\limits_0^{L_x}
        \bar{\rho}u_{al}^2\;\mathrm{d}x\mathrm{d}z}
        \label{eq:ahat_def}
\end{equation}
If $\bm{u} = \bm{u}_{al}$ over $z \in [z_b, z_t], \forall t$, then $\hat{A}_i(t) =
1$. The energy norm is a traditional normalization choice such that the $\bm{u}
\propto e^{z/2H}$ is evenly weighted throughout the integration region.

For a simulation satisfying $\xi_z k_z \ll 1$ everywhere, we
expect $\hat{A}_i(t) = 1$ when integrated between the forcing and damping zones,
i.e.\  $z_b \gtrsim z_0, z_t \lesssim z_T$ ($z_0, z_T$ are defined in
...
(using $z_t = z_T - \Delta z$ instead does not change the results). The
resulting measurement of $\hat{A}_i(t)$ is shown in \autoref{fig:lin_amps}.
\begin{figure}
    \centering
...
A second prediction of the analytical theory is that the horizontal momentmum
flux $F(z, t)$ should be spatially flat between the forcing zone where it is
generated and the damping zone where it is dissipated (\autoref{eq:F_def}). We
make may a slightly more accurate estimate of $F$ the flux carried in the IGW by
directly substituting $\bm{u}_{al}$ into \autoref{eq:F_def}. Calling this
estimate $F_{al}$, we may then measure the agreement of our simulation with
analytical expectation by computing
\begin{equation}
    \hat{F}(z, t) \equiv \frac{\int\limits_0^{L_x}\rho u_xu_z\;\mathrm{d}x}{
        \int\limits_{0}^{L_x} \bar{\rho} u_{x}u_{z}\;\mathrm{d}x}.
        \label{eq:sbm_def}
\end{equation}
...
that $\xi_z (z_0) k_z = 0.1$ in the forcing zone, which implies $\xi_z(z) k_z
\gtrsim 1$ before $z_T$ the upper damping zone. The dissipation parameter $\nu$
was varied across the various simulations. To quantify $\nu$, we define Reynolds
number
\begin{equation}
    \mathrm{Re} = \frac{LU}{\nu} \equiv \frac{\omega}{\nu k_{z}^2}.
        \label{eq:re_def}
...
% TODO generate a video for the 0.1 case
A full video of our higher-resolution run at $N_x = 1024, N_z = 4096,
\mathrm{Re} = 2048$ is available
online\footnote{http://www.princeton.edu/~lecoanet/data/breaking\_wave.mov}.
We take this to be our fiducial simulation for the remainder of this paper,
though other simulations show qualitatively identical behavior. Slices of
...
\begin{figure}
    ...
    and \autoref{eq:sbm_def}. $\overline{U}$ is given in units of $c_{ph, x} =
    \frac{\omega}{k_{x}}$ the critical layer flow velocity (see
    \autoref{ss:crit_layer}). The propagation of the critical layer towards
    lower $z$ and sharp deposition of $F$ at the critical layer are
...
reflected wave with wave vector $\bm{k} = k_{x}\bm{x} + k_{z}\bm{z}$, and
...
over multiple periods of the base IGW (specifically, averaged over approximately
$8\pi/\omega$). We can then define the amplitude reflectivity
\begin{equation}
    \mathcal{R}_A(t) \equiv \frac{\ev{\hat{A}_r}(t)}{\ev{\hat{A}_i}(t)}
        .\label{eq:Ra_def}
\end{equation}
% 7/N per timestep, 13 timestep smoothing kernel, 24/N period
Angle brackets denote time averaging here.
...
apparent but the mean values seem to have temporally converged.
...
\begin{figure}
    ...
    $16$ and $84$ percentile values. Fmall horizontal displacements are made for
...
introduce $\varpi = \frac{P}{\rho}$ instead, then mandate $\bar{\rho}, \varpi_0$
background fields satisfy hydrostatic equilibrium $\bm{\nabla}\varpi_0 +
\varpi_0 \bm{\nabla}\bar{\rho} + g\bm{z} = 0$. Taking isothermal stratification,
we find $\varpi_0 = gH$. We further change variables to $\Upsilon = \ln \rho -
\ln \bar{\rho}$ and $\varpi_1 = \varpi - \varpi_0$ deviations from the background
state to obtain a system of equations at most quadratic in fluid fields:
\begin{subequations}\label{se:nl_var}
    \begin{align}
        \bm{\nabla} \cdot \bm{u}_1 &= 0,\\
        \pd{\Upsilon}{t} + \p{\bm{u}_1 \cdot \bm{\nabla}} \Upsilon
            - \frac{u_z}{H} &= 0,\\
        \pd{u_{1x}}{t} + \p{\bm{u}_1 \cdot \bm{\nabla}}u_{1x}
            + \pd{\varpi_1}{x} + gH\pd{\Upsilon}{x}
            + \varpi_1 \pd{\Upsilon}{x} &= 0,\\
        \pd{u_z}{t} + \p{\bm{u}_1 \cdot \bm{\nabla}}u_z
...
\begin{subequations}
    \begin{align}
        \bm{\nabla} \cdot \bm{u} &= 0,\\
        \partial_t \rho + \bm{\nabla} \cdot (\rho \bm{u} - \nu
            \bm{\nabla}(\rho - \bar{\rho})) &= 0,\label{eq:visc_cons_mom}\\
...
\begin{subequations}
    \begin{align}
        ...
        - 2\nu \p{\p{\p{\bm{\nabla}\Upsilon} \cdot \bm{\nabla}}\bm{u} -
            \frac{1}{H}\partial_z \bm{u}} - \frac{\varpi_1}{H} &= 0.
    \end{align}
\end{subequations}
Hydrostatic equilibrium is still $\bm{\nabla} \varpi_0 + g\bm{z} = 0$ where
$\rho = \bar{\rho}, \bm{u} = 0$. Including the damping layers and forcing terms as
...
\begin{subequations}
    \begin{align}
        ...
        \partial_t \Upsilon - \frac{u_z}{H}
            ...
            & - \p{\bm{u} \cdot \bm{\nabla}}\Upsilon
                -\Gamma(z) \Upsilon
                + \frac{F}{\bar{\rho}(z)}e^{-\frac{(z - z_0)^2}{2\sigma^2}}
                    \cos \p{k_xx - \omega t},\\
