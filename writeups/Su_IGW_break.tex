...
\begin{align*}
    ...
    -i\omega \Upsilon - \frac{u_{z}'}{H} &=
        Ce^{-\frac{(z - z_0)^2}{2\sigma^2}}.
\end{align*}
We have replaced all partial derivatives with regular derivatives. Compare to
the left hand side of \autoref{se:dedalus_eqs} below. This can be recast solely
in terms of $u_{1z}$ as
\begin{align*}
    \omega^2\p{
        k_x^2u_{z}' + \frac{1}{H}\rd{u_{z}'}{z} - \rtd{u_{z}'}{z}
    } - u_{z}'N^2k_x^2 =
...
We compute the solution to the inhomogeneous ODE by the method of variation of
parameters. The homogeneous solutions are of form $u_{z,\pm}'(z) =
e^{\frac{z}{2H} \pm ik_zz}$ where $k_z$ satisfies the dispersion relation
\autoref{eq:disp_rel}. The Wronskian can be computed
\begin{equation}
