...
the forcing location. We choose $L_z = 10H$ to give $\sim e^3$ amplitude growth
between the damping zones. We force at $z_0 = 2H = 0.2L_z$ within width
$\sigma = 0.078H$, sufficiently far from the lower damping zone and permit
sufficient room for the upward moving wave to grow as $\propto e^{z/2H}$.
Finally, we want similar grid spacing $\frac{L_x}{N_x} \sim \frac{L_z}{N_z}$,
guided by the intuition that turbulence is approximately isotropic, so we use
$L_x = 4H$ and $N_z / N_x = 4$.
...
\end{equation}
\autoref{fig:lin_fluxes} shows that $\hat{F}(z)\approx 1$ between $z_0$ and
$z_T$, as expected.
\begin{figure}
    \centering
...
In \autoref{fig:nl_fluxes}, we plot $\overline{U}$ and $\hat{F}$ as a function
