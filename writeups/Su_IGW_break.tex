...
are\footnote{\textcolor{Corr}{These equations arise by enforcing fluid
incompressibility ($\mdil{\rho}{t} = 0$ or the adiabatic index $\gamma \to
\infty$) in the Euler equations. They also can be derived from the commonly used
``sound-proof'' \emph{anelastic equations} \citep[see e.g.][]{ogura1962scale,
sutherland1} in the limit where (i) the density stratification is weak ($H \gg
L$ for perturbation wavelength $L$) and (ii) the fluid is incompressible (note
the anelastic equations are defined for arbitrary $\gamma$). Under condition
(i), the anelastic approximation reduces to the Boussinesq approximation
\citep{klein2009asymptotics}. Our equations permit a weak but nonzero density
stratification and can be interpreted as ``locally Boussinesq''. Condition (ii)
is motivated by WD fluid properties and, more generally, simplicity, as the
dynamical effect of wave breaking is likely to be similar for different sets of
fluid equations.}}
