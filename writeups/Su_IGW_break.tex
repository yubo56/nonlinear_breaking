...
\begin{abstract}
    ...
    tidal process involves the excitation of internal gravity waves (IGWs) that
    ...
    flow) and the upper ``synchronized'' region (with the mean flow velocity
    equal to the horizontal wave phase speed). We find that the indefinite
    steepening of this critical layer is suppressed by Kelvin-Helmholtz
    instabilities, and the width of the layer is determined by the Richardson
...
plane-parallel stratified atmosphere (stellar enevelope). We use the
...
\begin{enumerate}
    ...
    \item The averaged horizontal momentum flux $F$ (in the $+\uv{z}$
        ...
        the linear solution (Eq.~\ref{eq:lin_sol}), this evaluates to
        \begin{equation}
...
\abs{\bm{k}}$. This can be rewritten using the Lagrangian displacement $\bm{\xi}
= \bm{u} / \p{-i\omega}$:
\begin{equation}
...
\begin{enumerate}
    ...
    \item $\nu$: Nonlinear effects transfer wave energy from the injection
        ...
        ranging from $10^{-11}$--$10^-4$ of the mode frequency \citep{fullerI}.
        This corresponds to a small effective viscosity or $\mathrm{Re} \gg
...
\begin{enumerate}
    ...
    \item The absorption of IGW momentum flux at the critical layer is
        ...
        flux in most simulations (see Fig.~\ref{fig:agg}). The reflected flux is
        carried away from the critical layer as both lowest-order reflected
        waves and waves with larger $z$ wavenumbers.
\end{enumerate}

\subsection{Discussion}

In this paper, we have studied the nonlinear behavior of IGWs with $\abs{k_x /
k_z} \sim 1$ in a plane-parallel geometry. Tidally excited IGWs in binary stars
have horizontal wave number $k_\perp \sim 1/R$ (where $R$ is the stellar radius)
much smaller than the radial wave number $k_r$. Strictly speaking, our results
may not directly apply to IGWs with $\abs{k_x/k_z}\ll 1$. However, the
qualitative behavior is likely to be similar, as the turbulence driving the
critical layer dynamics occurs at scales significantly smaller than either
$1/k_x$ or $1/\abs{k_z}$. Simulating IGWs with $k_x \ll \abs{k_z}$ is more
challenging numerically and we defer its exploration to future work.
...
geometry, they found no evidence for reflected waves, contrary to our result. It
is plausible that the discrepancy arises due to the differing causes of wave
